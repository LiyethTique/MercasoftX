...
const WriteTable = ({ titles, data, fileName = 'Gestionar Responsable' }) => {
    ...
    const exportToPDF = () => {
        ...
        doc.save(`${fileName}.pdf`); // Guardar con el nombre del archivo
    };
    ...
    const exportToExcel = () => {
        ...
        const ws = XLSX.utils.aoa_to_sheet([
            ...
            titles,          // Encabezados de las columnas
            ...filteredData  // Datos
        ]);
        const wb = XLSX.utils.book_new();
        XLSX.utils.book_append_sheet(wb, ws, 'Sheet1');
        XLSX.writeFile(wb, `${fileName}.xlsx`); // Guardar como archivo Excel
    };
    ...
    const exportToSQL = () => {
        ...
        filteredData.forEach(row => {
            ...
            sql += `INSERT INTO table_name (${titles.join(', ')}) VALUES (${values});\n`;
        });

        const blob = new Blob([sql], { type: 'text/sql' });
        const link = document.createElement('a');
        link.href = URL.createObjectURL(blob);
        link.download = `${fileName}.sql`; // Descargar archivo SQL
        link.click();
    };
