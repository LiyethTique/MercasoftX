...
function WriteTable({ titles, data }) {
  ...
  const [formType, setFormType] = useState('venta'); // Solo 'venta' ahora
  const [formData, setFormData] = useState({});
  const [buttonForm, setButtonForm] = useState('Enviar');
  const URI = "/api/venta/"; // Cambia esto según la URL de tu API
  const moduleName = "Gestionar venta"; // Nombre del módulo actualizado
  ...
  return (
    <div className="container mt-4">
      ...
      <div className={`modal fade ${isFormVisible ? 'show d-block' : ''}`} style={{ display: isFormVisible ? 'block' : 'none', backgroundColor: 'rgba(0,0,0,0.5)' }}>
